...
## 先抓思想：VLA 微调不是调风格，而是对齐身体和任务

VLA post-training 的精髓不是“用 LoRA 省显存”，而是回答一个部署问题：

> 基础 VLA 学到的是通用机器人经验，但真实使用必须把它对齐到目标机器人、目标相机、目标动作空间和目标任务分布。

LLM 微调常常关注回答格式、知识和偏好；VLA 微调还要处理动作是否可执行。这个差别决定了本文的阅读重点。

| 层级 | VLA 微调的回答 | 为什么重要 |
| --- | --- | --- |
| 思想矛盾 | base VLA 有通用能力，但目标机器人接口不同 | post-training 把通用能力对齐到具体 embodiment |
| 核心创新 | 参数高效更新、数据映射、normalization stats、评估闭环一起看 | LoRA 只是其中一环，不解决动作契约 |
| 架构结果 | 冻结部分 backbone，训练 adapter / LoRA / action expert 或少量模块 | 降低训练成本，同时尽量保留基础能力 |
| 证据重点 | base vs finetuned、少样本曲线、ID/OOD 成功率、失败类型 | 判断微调是否真的带来可执行收益 |
| 局限 | loss 下降不代表 rollout 成功，norm stats 错会直接毁掉动作 | 解释为什么微调篇必须比普通 NLP fine-tuning 更强调接口检查 |

所以这篇文章的主角不是 LoRA rank，而是 observation/action/normalization/evaluation contract 是否一起对齐。
