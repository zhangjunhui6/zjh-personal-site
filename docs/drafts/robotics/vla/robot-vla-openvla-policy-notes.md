...
## 先抓思想：把 robot action 改写成 token prediction

OpenVLA 的核心思想不是“把 Llama 2 接到机器人上”，而是提出一个很强的假设：

> 如果 VLM 已经擅长在图像和语言条件下预测 token，那么机器人动作也可以被离散化成 token，让 VLM 直接成为 robot policy。

这个假设解决的是 VLA 的第一条主线：如何复用视觉语言预训练能力来输出动作。OpenVLA 的创新点要从这个思想出发理解。

| 层级 | OpenVLA 的回答 | 为什么重要 |
| --- | --- | --- |
| 思想矛盾 | VLM 懂视觉语言，但机器人需要连续动作 | 把动作离散化，让动作进入语言模型的生成接口 |
| 核心创新 | 7-DoF 动作逐维 256-bin 离散，并映射到 Llama tokenizer 的最后 256 个 token | 不另起一个 action head，而是复用 next-token prediction |
| 架构结果 | DINOv2 + SigLIP 视觉编码、MLP projector、Llama 2 backbone、action de-tokenizer | 架构服务于“动作即 token”这个假设 |
| 证据重点 | BridgeData / WidowX 等任务的 success rate、泛化轴、LoRA/quantization 分析 | 证明它是可检查、可适配的开放基线，而不只是 demo |
| 局限 | 动作离散化、逐 token 解码、频率和 action stats 依赖 | 解释为什么 π0 / SmolVLA 会转向 continuous action expert |

所以读 OpenVLA 的顺序应该是：先理解 action tokenization 这个路线选择，再读 Figure 1 的架构；否则 Figure 1 只会像一张普通 VLM 模型图。
