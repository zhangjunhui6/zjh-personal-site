...
## 先抓思想：机器人动作不是语言 token，而是连续轨迹

π0 的精髓是对 OpenVLA 式路线提出另一个判断：

> 视觉语言理解可以继续借助 VLM，但机器人动作本身不必被压成离散 token；它更像一段条件连续轨迹，应该由 action expert 生成。

这就是为什么 π0 的架构里会出现 action expert、flow matching 和 action chunk。它不是为了让模型图更复杂，而是在回答“机器人控制到底该不该服从语言模型 token 生成范式”这个思想问题。

| 层级 | π0 的回答 | 为什么重要 |
| --- | --- | --- |
| 思想矛盾 | VLM 擅长语义理解，机器人动作却是高频连续控制 | VLM 和 action expert 分工，而不是强行把动作语言化 |
| 核心创新 | 用 flow matching 生成 `H=50` 的连续 action chunk | 一次生成未来动作序列，适配更高频和更灵巧的控制 |
| 架构结果 | PaliGemma backbone + 约 300M action expert + 多机器人数据混合 | 语义上下文和连续动作生成解耦 |
| 证据重点 | 多机器人配置、长程灵巧任务、pre-training 与 post-training 数据配方 | 证明它目标是 general robot control，而不是单个 benchmark 刷分 |
| 局限 | 采样步数、延迟、chunk staleness、embodiment adapter 和安全系统仍然关键 | 解释为什么 openpi 实验必须检查 action horizon、state/action dim 和 transforms |

所以 π0 的读法不是“学一个 flow matching 公式”这么窄。更重要的是理解：它把 VLA 从 action-token policy 推向 continuous action generation policy。
