...
LeRobot 的精髓不是“装一个库”，而是回答 VLA 自学里最现实的问题：

> 论文里的 observation、action、policy、processor、normalization 和 evaluation，如何变成可检查、可复现的工程对象？

如果没有这座桥，读者可能理解了 OpenVLA 或 SmolVLA 的思想，却在数据字段、action shape、norm stats 或 eval wrapper 上卡住。LeRobot 的价值就在于把这些隐性契约显式化。

| 论文概念 | LeRobot 里的落点 | 为什么重要 |
| --- | --- | --- |
| 机器人轨迹 | episode / frame / dataset shards | 机器人数据不是独立图片，而是时间序列 |
| observation contract | `observation.images.*`、`observation.state`、feature schema | 决定模型实际看到什么 |
| action contract | `action` feature、shape、normalization stats | 决定模型输出能否被环境执行 |
| policy contract | config、checkpoint、processor、inference API | 决定论文架构如何落到代码 |
| evaluation contract | environment wrapper、rollout loop、success metric | 决定结果是否可复查 |

所以这篇文章的重点不是 Docker 命令，而是让你以后看到任何 VLA repo，都知道先检查哪些接口。
