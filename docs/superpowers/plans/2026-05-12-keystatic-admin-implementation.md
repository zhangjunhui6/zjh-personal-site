...
```bash
PUBLIC_KEYSTATIC_STORAGE=local KEYSTATIC_STORAGE=local npm run dev -- --host 127.0.0.1
```
...
Expected: Keystatic admin loads without GitHub login because `PUBLIC_KEYSTATIC_STORAGE=local`.
...
After Keystatic generates local `.env` values, verify `.env` has `PUBLIC_KEYSTATIC_STORAGE=github`, `KEYSTATIC_STORAGE=github`, and non-empty values for `KEYSTATIC_GITHUB_CLIENT_ID`, `KEYSTATIC_GITHUB_CLIENT_SECRET`, `KEYSTATIC_SECRET`, and `PUBLIC_KEYSTATIC_GITHUB_APP_SLUG`.
...
- `PUBLIC_KEYSTATIC_STORAGE`: set to `github`.
...
In Cloudflare Pages project settings for `zjh-personal-site`, add the six production variables from the local `.env` file:
